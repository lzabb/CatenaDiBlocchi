...
transactions_list =[
              [{'from' : 'toni', 'to': 'zabba', 'amount' : int(np.random.randint(9,10, 1))},
               ...
               {'from' : 'zabba', 'to': 'toni', 'amount' : int(np.random.randint(0,10, 1))}],
              [{'from' : 'toni', 'to': 'zabba', 'amount' : int(np.random.randint(0,10, 1))},
               {'from' : 'toni', 'to': 'zabba', 'amount' : int(np.random.randint(0,10, 1))}, 
               {'from' : 'zabba', 'to': 'toni', 'amount' : int(np.random.randint(0,10, 1))}], 
              [{'from' : 'toni', 'to': 'zabba', 'amount' : int(np.random.randint(0,10, 1))},
...
# The following, for each round checks first if the transactions for each 'from' 
# have the required balance to choose which transactions to accept, 
# then the accepted transactions go in the next block.          
